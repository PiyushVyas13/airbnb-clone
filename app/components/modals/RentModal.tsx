    ...
    if(step === STEPS.PRICE) {
        bodyContent = (
            <div className="flex flex-col gap-8">
                ...
                <Input id="price" label="Price" register={register} errors={errors} required formatPrice disabled={isLoading} type="number" />
            </div>
        )
    }

    return (
        <Modal  title="Airbnb your home" isOpen={rentModal.isOpen} onClose={rentModal.onClose} onSubmit={handleSubmit(onSubmit)} actionLabel={actionLabel} secondaryLabel={secondaryActionLabel} secondaryAction={step === STEPS.CATEGORY ? undefined : onBack} body={bodyContent}/>
    );
};
